...
$(function () {
  ...
  var andCityAreaStr = null; // 搜索条件(城市+区域)数据

  var andCityAreaFunctionArr = []; // 搜索条(城市+区域+功能)件数据暂存数组
  var andCityAreaFunctionStr = null; // 搜索条件(城市+区域+功能)数据

  // 根据搜索起始页条件 搜索
  var searchStr = decodeURIComponent(location.search); //获取地址栏数据(搜索起始页条件)
  initSearch(searchStr); // 初始搜索
  ...
  $('#getAreaTypeList').on('click', 'li', function () { // 选择相应区域
    filterAreaId = [];
    var self = $(this);
    var selfId = self.attr('data-curid'); // 获取当前区域的 id
    if (selfId === 'no') {
      filterAreaId = [];
    } else {
